...
# This works for both local execution (run from project root) and Docker (WORKDIR is /app).
...
def generate_yearly_rankings(year: int) -> Tuple[Dict, Optional[str]]:
    ...
    annual_scores_path = os.path.join(PROCESSED_DATA_DIR, ANNUAL_SCORES_FILE)
    classifications_path = os.path.join(REFERENCE_DATA_DIR, CLASSIFICATIONS_FILE)
    message = None
...
if __name__ == '__main__':
    ...
    dummy_data_path = os.path.join(PROCESSED_DATA_DIR, ANNUAL_SCORES_FILE)
